...
const orderModel34 = require("../../models/orderModel34");
const crypto = require("crypto");
const orderModel34Abd = require("../../models/orderModel34-abd");
...
router.post("/create-order", async (req, res) => {
  ...
  try {
    ...
    const existingOrder = await orderModel34.findOne({ orderId });
    if (existingOrder) {
    ...
    const order = await orderModel34.create(payload);
...
router.get("/get-orders", async (req, res) => {
  const orders = await orderModel34.find({}).sort({ createdAt: -1 });
  return res.status(200).json({ success: true, data: orders });
});

/**
 * POST /api/lander34/create-order-abd
 */
router.post("/create-order-abd", async (req, res) => {
  ...
  try {
    ...
    const order = await orderModel34Abd.create(payload);
    return res.status(200).json({ success: true, data: order });
  } catch (error) {
...
/**
 * DELETE /api/lander34/delete-order-abd/:id
 */
router.delete("/delete-order-abd/:id", async (req, res) => {
  ...
  try {
    const order = await orderModel34Abd.findByIdAndDelete(id);
...
/**
 * GET /api/lander34/get-orders-abd
 */
router.get("/get-orders-abd", async (req, res) => {
  const orders = await orderModel34Abd.find({}).sort({ createdAt: -1 });
  return res.status(200).json({ success: true, data: orders });
});
...
router.get("/get-orders/main", async (req, res) => {
  ...
  const orders = await orderModel34
    .find(query)
    .sort({ createdAt: -1 })
    .skip((page - 1) * limit)
    .limit(limit);
...
router.get("/get-orders/abd-main", async (req, res) => {
  ...
  const orders = await orderModel34Abd
    .find(query)
    .sort({ createdAt: -1 })
    .skip((page - 1) * limit)
    .limit(limit);
...
router.patch("/delivery-status/:id", async (req, res) => {
  try {
    ...
    const order = await orderModel34.findByIdAndUpdate(
      id,
